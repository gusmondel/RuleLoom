...
The term ILP is deliberately scoped: RuleLoom 0.10.0 propositionalizes each change
...
language branches. New schema-v5 initializations default to
the configurable `generic_changes@3`; schema-v3/v4/v5 `configured_paths@1` adds a canonical
...
## First public falsification attempt

The preregistered Apache Airflow experiment is the first direct empirical test
of this product thesis in RuleLoom. Using exact PR-opening snapshots and a later
independent `changes_requested` target, the Horn learner emitted no qualifying
rule and scored holdout MCC `0.000`. The best train-selected literal scored
`0.081`; a supplementary Boolean logistic baseline scored `0.136` with only
`0.159` precision. The frozen success criterion failed. A source-continuity
audit added after inspecting the first metrics found 42 missing GH Archive
hours, invalidated that attempt, and conservatively reclassified every
gap-crossing negative before the clean rerun reported here.

This does not refute ILP generally. It does falsify the narrower claim that the
frozen coarse path/change-shape vocabulary predicts this target at useful
precision in the retained Airflow cohort. Retention also differed between
positive and negative archive units, limiting generalization. The complete
...
|---|---|---|---|---|
| Kamei et al., *A large-scale empirical study of just-in-time quality assurance* (2013) | Peer-reviewed, IEEE TSE 2013 | Change-level defect prediction works with fourteen Git-derivable metrics in four families: size, diffusion, history and experience; history and experience carry signal beyond size. | `generic_changes@4` adds the experience, ownership, rework-history, entropy and timing families so the vocabulary covers all four; the logistic probe baseline measures the vocabulary independently of the rule learner. | It does not validate any particular threshold, any repository, or the rework proxy as a defect label. |
| Mockus and Weiss, *Predicting risk of software changes* (2000); Bird et al., *Don't touch my code!* (2011) | Peer-reviewed, Bell Labs Tech. J. 2000; ESEC/FSE 2011 | Developer experience lowers change risk; files with many minor contributors have more defects. | `author_low_experience`, `author_new_to_area` and `touched_files_many_authors` are computed from privacy-preserving author hashes over the strictly earlier observation stream. | Author hashes cannot distinguish bots from people, and the findings concern defects, not rework. |
| Kim, Zimmermann, Whitehead and Zeller, *Predicting faults from cached history* (2007) | Peer-reviewed, ICSE 2007 | Recently faulty files are likely to be faulty again (temporal locality). | `touches_recently_reworked_file` reads persisted `rework` events that landed strictly before the change. | The cache was validated on fix commits; RuleLoom's rework events are a line-content proxy. |
| Hassan, *Predicting faults using the complexity of code changes* (2009); Eyolfson, Tan and Lam, *Do time of day and developer experience affect commit bugginess?* (2011) | Peer-reviewed, ICSE 2009; MSR 2011 | Scattered changes and late-night commits are associated with more defects. | `change_entropy_high` (normalized churn entropy at least 0.75 over at least three files) and `authored_off_hours` (weekend or 22:00 to 06:00 in the timestamp's own offset). | Both effects are small and project-dependent; neither is causal. |
...
| Law et al., *FastLAS* | Peer-reviewed, AAAI 2020 | FastLAS supports user-defined hypothesis scoring; the evaluated access-control tasks showed that scoring can target domain-specific objectives and that the system was faster and more scalable than compared ILP systems. | A future engine could encode asymmetric interruption and missed-risk costs explicitly. | RuleLoom 0.10.0 does not implement FastLAS, and access-control results do not establish coding-agent value. |
...
| Tabassum et al., *Cross-Project Online Just-In-Time Software Defect Prediction* | Peer-reviewed, IEEE TSE 2023 | Combining incoming cross-project and within-project examples improved early and some later G-mean periods in the evaluated online projects. | Cross-project evidence is a credible future cold-start experiment with explicit source provenance and target-repository confirmation. | Distribution shift and false-alert costs remain; RuleLoom 0.10.0 intentionally does not pool repositories automatically. |
...
variable, every literal refers to the same change observation; the 0.10.0
learner does not discover relations among entities, although reviewed
`missing_partner_*` predicates instantiate one relational pattern per frozen
pair.
...
artifact. Version 0.10.0 accepts only one non-recursive learned rule, disables
...
response to review. Version 0.10.0 represents one logical `ChangeUnit` with an
...
truth. RuleLoom v0.10.0 therefore implements three evidence grades:
...
It is not a claim established by the cited research literature. RuleLoom v0.10.0
...
- a predicate proposed from co-change or ownership structure is meaningful,
...
| Coarse Booleans cap precision at the base rate of large equivalence classes | Outcome-blind proposer that instantiates hotspots, owner areas, and co-change pairs as reviewed `generic_changes@3` predicates, bounded to the pre-holdout window |
| A marginal top-K prefix hides predicates that matter only in conjunction | Beam search over every eligible predicate with logistic-weight ordering; exhaustive mode retained for legacy schemas |
