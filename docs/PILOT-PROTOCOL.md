...
| Evidence profile | Schema version plus one exact `pack@version`; use schema-v5 `generic_changes@3` (with an empty or reviewed `pack_config`) unless the experiment has pre-registered a supported specialized or configurable pack |
...
| Fixed holdout boundary | Required aware `evaluation.test_start_at` for schema v4 and v5; observations before it are eligible for probe/train and observations at/after it form the untouched holdout. Labels unavailable at that instant are embargoed from training. |
...
| Horn gate | Absolute precision for legacy schemas or schema-v4/v5 relative lift plus alert-rate threshold; schema v5 additionally freezes the Horn 0.6 search controls (beam, Wilson lower bound, temporal consistency, prune window, permutation null, tree seeds). Preserve near-miss limit, hypotheses examined, and the permutation-null report. |
...
Windows, because its storage lock uses POSIX `fcntl`. `generic_changes@3` is the
schema-v5 default and provides language-neutral change-shape, path-role,
...
`schema_version: 5`; `configured_paths@1` is also supported under schema v5. Freeze its
...
The v0.10.0 archive adapter supports the explicit `github.com` host. By default it
...
RuleLoom v0.10.0 ships a local GitHub Action/webhook capture substrate and bounded
